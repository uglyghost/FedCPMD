...
class FedSoftPerServer(FedAvgServer):
    def __init__(
            self,
            algo: str = "FedCPMD_W_40_0.1_4_layer",
            args: Namespace = None,
            unique_model=False,
        ...
        self.phase1 = 40
        self.metrics1, self.metrics2, self.metrics3, self.metrics4 = {
            "train_before": [],
            "train_after": [],
            "test_before": [],
            "test_after": [],
    ...
    def train(self):
        ...
        if selected_clients_set == all_clients:
            ...
            if self.phase1 > 50:
                self.phase1 = 50
            for E in range(1, (200 - self.phase1) * 4 + 1):  # E的范围从1到300
                # 1. ###################################################################################################
                ...
                #     self.cluster_name =layer_name
                #     self.number = len(clients_in_cluster)
                ...
                #     #     random.sample(
                #     #         clients_in_cluster,
                #     #         math.ceil(len(clients_in_cluster) / 3) if len(clients_in_cluster) <= 50 else
                #     #         math.ceil(len(clients_in_cluster) / 6)
                #     #     )
                #     #     for _ in range(150)
                #     # ]
                #     self.client_sample_cluster_stream = [
                ...
                #         ]
                #     self.selected_clients = self.client_sample_cluster_stream[current_E]
                #     self.weight_matrix_cluster = torch.eye(len(clients_in_cluster), device=self.device)
                #     # self.current_epoch = current_E + self.count_if_executed + 1
                ...
                #             new_parameters=client_pers_params,
                #             return_diff=True,
                #             verbose=((current_E + 1) % self.args.verbose_gap) == 0,
                #         )
                #         client_params_cache.append(client_params)
                #         delta_cache.append(delta)
                #     self.update_client_params(client_params_cache)
