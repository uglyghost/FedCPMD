...
class FedSoftPerServer(FedAvgServer):
    def __init__(
            self,
            algo: str = "FedCPMD_B_50_0.1_0.1",
            # algo: str = "FedCPMD_B_client_layer",
            args: Namespace = None,
            unique_model=False,
    ...
    def train(self):
        ...
        for E in fedsper_progress_bar:  # 前n轮
            if selected_clients_set != all_clients or self.current_epoch < self.phase1:
                ...
                for client_id in self.selected_clients:
                    ...
                    if not self.two_smallest_elements[client_id]:
                        self.trainer.personal_params_name.append(
                            ...
                            any(ele in name for ele in 'classifier')
                        )
                    else:
                        self.trainer.personal_params_name.append(
                            name for name in self.model.state_dict() if
                ...
                print(self.two_smallest_elements)
        ...
        if selected_clients_set == all_clients:
            ...
            #
            # # 少于5个的类，从最大类中随机选几个补齐至5个
            # longest_layer = max(clustered_clients, key=lambda x: len(clustered_clients[x]))
            # # 如果layer_name不存在，则创建一个空列表
            # if 'classifier' not in clustered_clients:
            #     clustered_clients['classifier'] = []
            # for layer_name, client_ids in clustered_clients.copy().items():
            #     # 判断列表长度是否小于5
            #     if len(client_ids) < 5 and layer_name == 'classifier':
            #         # 计算需要补充的客户端数量
            #         num_to_add = 5 - len(client_ids)
            #         # 从其他最大类的客户端中随机选择几个进行补充
            #         clients_to_add = random.sample(clustered_clients[longest_layer], num_to_add)
            #         # 将补充的客户端添加到当前小类中
            #         clustered_clients[layer_name].extend(clients_to_add)
            #         # 从最大类中删除已添加的客户端
            #         for client in clients_to_add:
            #             clustered_clients[longest_layer].remove(client)
