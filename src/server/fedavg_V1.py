...
from src.client.fedavg_V1 import FedAvgClient
...
class FedAvgServer:
    def __init__(
        ...
        random_init_params, self.trainable_params_name = trainable_params(
            self.model, detach=False, requires_name=True
        )
        ...
        if (
            ...
            self.client_trainable_params = [
                trainable_params(self.model, detach=False) for _ in self.train_clients
            ]
    ...
    def train_one_round(self):
        ...
        for client_id in self.selected_clients:
            client_local_params = self.generate_client_params(client_id, client_id)
            (
                delta,
                weight,
    ...
    def log_info(self):
        ...
        for label in ["train", "test"]:
            ...
            if (label == "train" and self.args.eval_train) or (
                ...
                # print("correct_before:" + str(correct_before))
                ...
                # print("correct_after:" + str(correct_after))
                ...
                # print("num_samples:" + str(correct_after))
    ...
    def log_info1(self):
        ...
        for label in ["train", "test"]:
            ...
            if (label == "train" and self.args.eval_train) or (
                ...
                self.metrics1[f"{label}_before"].append(acc_before)
                self.metrics1[f"{label}_after"].append(acc_after)
    ...
    def log_info2(self):
        ...
        for label in ["train", "test"]:
            ...
            if (label == "train" and self.args.eval_train) or (
                ...
                self.metrics2[f"{label}_before"].append(acc_before)
                self.metrics2[f"{label}_after"].append(acc_after)
    ...
    def log_info3(self):
        ...
        for label in ["train", "test"]:
            ...
            if (label == "train" and self.args.eval_train) or (
                ...
                self.metrics3[f"{label}_before"].append(acc_before)
                self.metrics3[f"{label}_after"].append(acc_after)
    ...
    def log_info4(self):
        ...
        for label in ["train", "test"]:
            ...
            if (label == "train" and self.args.eval_train) or (
                ...
                self.metrics4[f"{label}_before"].append(acc_before)
                self.metrics4[f"{label}_after"].append(acc_after)
    ...
    def run(self):
        ...
        if self.args.save_metrics:
            ...
            accuracies1 = []
            labels1 = []
            for label, acc in self.metrics1.items():
                if len(acc) > 0:
                    accuracies1.append(np.array(acc).T)
                    labels1.append(label)
            pd.DataFrame(np.stack(accuracies1, axis=1), columns=labels1).to_csv(
                OUT_DIR / self.algo / f"{self.number1}_{self.cluster_name1}_{self.args.dataset}_acc_metrics.csv",
                index=False,
            )

            accuracies2 = []
            labels2 = []
            for label, acc in self.metrics2.items():
                if len(acc) > 0:
                    accuracies2.append(np.array(acc).T)
                    labels2.append(label)
            pd.DataFrame(np.stack(accuracies2, axis=1), columns=labels2).to_csv(
                OUT_DIR / self.algo / f"{self.number2}_{self.cluster_name2}_{self.args.dataset}_acc_metrics.csv",
                index=False,
            )

            accuracies3 = []
            labels3 = []
            for label, acc in self.metrics3.items():
                if len(acc) > 0:
                    accuracies3.append(np.array(acc).T)
                    labels3.append(label)
            pd.DataFrame(np.stack(accuracies3, axis=1), columns=labels3).to_csv(
                OUT_DIR / self.algo / f"{self.number3}_{self.cluster_name3}_{self.args.dataset}_acc_metrics.csv",
                index=False,
            )

            # accuracies4 = []
            # labels4 = []
            # for label, acc in self.metrics4.items():
            #     if len(acc) > 0:
            #         accuracies4.append(np.array(acc).T)
            #         labels4.append(label)
            # pd.DataFrame(np.stack(accuracies4, axis=1), columns=labels4).to_csv(
            #     OUT_DIR / self.algo / f"{self.number4}_{self.cluster_name4}_{self.args.dataset}_acc_metrics.csv",
            #     index=False,
            # )
